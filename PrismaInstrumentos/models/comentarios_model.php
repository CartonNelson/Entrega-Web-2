...
class comentarios_model extends model
{
  ...
  function crearComentario($usuario,$texto,$rate){
    $sentencia = $this->db->prepare("INSERT INTO comentario(user,texto,rate)VALUES(?, ?, ?)");
    $sentencia->execute( array($usuario,$texto,$rate));
    return $sentencia->rowCount();
